...
export function renderCustomHelp(): string {
  const lines: string[] = [
    '',
    `  ${BOLD.cyan('Niriksh Agent')}`,
    `  ${DIM('AI Usage Intelligence Platform')}`,
    '',
    `  ${BOLD('Usage:')}`,
    `    niriksh ${GREEN('<command>')} ${DIM('[options]')}`,
    '',
    `  ${BOLD('Commands:')}`,
    '',
    ...
    `    ${CYAN('sync')}         Sync usage to Niriksh Cloud`,
    ...
    `  ${BOLD('Examples:')}`,
    '',
    `    niriksh                    ${DIM('# open the dashboard')}`,
    `    niriksh report -p month    ${DIM('# monthly report')}`,
    `    niriksh status             ${DIM('# quick status')}`,
    `    niriksh export -f json     ${DIM('# export as JSON')}`,
    `    niriksh optimize           ${DIM('# find waste')}`,
    `    niriksh sync --once        ${DIM('# one-time cloud sync')}`,
    `    niriksh doctor             ${DIM('# run diagnostics')}`,
    '',
  ]
  return lines.join('\n')
}
