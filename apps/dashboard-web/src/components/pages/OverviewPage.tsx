...
function InsightCard({ icon, title, value, subtitle, iconBg = 'bg-primary/10' }: InsightCardProps) {
  return (
  ...
  return (
    <div className="flex items-center gap-3 rounded-lg px-3 py-2.5 transition-colors hover:bg-muted/50">
      ...
      <div className="text-right">
        <p className="text-xs font-medium">{formatTokens(item.tokens)} tokens</p>
...
export function OverviewPage() {
  ...
  const fastestGrowingProvider = useMemo<ProviderAnalytics | undefined>(() => {
    ...
    // Approximate growth by comparing first/second half share
  ...
  // Most active team (approximate: top user as proxy for team)
  const mostActiveTeam = useMemo(() => {
    if (!users || users.length === 0) return undefined;
    const top = users.reduce((best, u) => (u.sessionCount > best.sessionCount ? u : best), users[0]);
    return top;
  }, [users]);

  // Recent activity (top 10 sessions simulated from users)
    ...
    <div className="space-y-8">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
        {isAnyLoading ? (
          ...
          <>
            <MetricCard
              icon={<Coins className="h-5 w-5 text-violet-600 dark:text-violet-400" />}
              label="Total Tokens"
      ...
      <div>
        <h2 className="mb-4 text-lg font-semibold tracking-tight">Top Insights</h2>
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          ...
          />
          <InsightCard
            icon={<UserCircle className="h-5 w-5 text-rose-600 dark:text-rose-400" />}
