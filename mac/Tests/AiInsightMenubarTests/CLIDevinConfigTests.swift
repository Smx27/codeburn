...
struct CLIDevinConfigTests {
    private func withTemporaryStore(_ body: (URL, AiInsightCLIConfigStore) throws -> Void) throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("aiinsight-devin-config-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer {
            try? FileManager.default.removeItem(at: root)
        }
        try body(root, AiInsightCLIConfigStore(homeDirectory: root.path))
    }
