...
private let menubarPeriodDefaultsKey = "AiInsightMenubarPeriod"
...
final class AppStore {
    ...
    var displayMetric: DisplayMetric = DisplayMetric(rawValue: UserDefaults.standard.string(forKey: "AiInsightDisplayMetric") ?? "") ?? .cost {
        didSet { UserDefaults.standard.set(displayMetric.rawValue, forKey: "AiInsightDisplayMetric") }
    }
    var dailyBudget: Double = UserDefaults.standard.double(forKey: "AiInsightDailyBudget") {
        didSet { UserDefaults.standard.set(dailyBudget, forKey: "AiInsightDailyBudget") }
    }
    ...
    func clearStaleLoadingIfNeeded() -> Bool {
        ...
        for (key, started) in staleLoading {
            NSLog("AiInsight: loading stuck for %ds on %@/%@ — auto-clearing",
                  Int(now.timeIntervalSince(started)), key.label, key.provider.rawValue)
            loadingCountsByKey[key] = nil
            loadingStartedAtByKey[key] = nil
            inFlightKeys[key] = nil
            if cache[key] == nil {
                lastErrorByKey[key] = "Refresh took longer than expected. AiInsight will keep retrying in the background."
            }
        }
        for (key, insertedAt) in staleInFlight {
            NSLog("AiInsight: orphaned in-flight key stuck for %ds on %@/%@ — clearing",
    ...
    private func invalidateStaleDayCache() {
        ...
        if cacheDate != today {
            ...
            lastErrorByKey.removeAll()
            cacheDate = today
            NSLog("AiInsight: reset menubar payload cache for new day %@", today)
        }
    }
    ...
    func setRecoveryExhausted(for label: String) {
        lastErrorByKey[currentKey] = "Could not load \(label). Check that the aiinsight CLI is installed and working."
    }
    ...
    private func refresh(key: PayloadCacheKey, includeOptimize: Bool, force: Bool = false, showLoading: Bool = false) async {
        ...
        if staleSeconds.isFinite, staleSeconds > 120 {
            NSLog("AiInsight: refresh attempt for stale key \(key.label)/\(key.provider.rawValue) — last success was \(Int(staleSeconds))s ago")
        }
        defer {
        ...
        do {
            ...
            if generationAtStart != payloadRefreshGeneration {
                NSLog("AiInsight: dropping fetch result for \(key.label)/\(key.provider.rawValue) — refresh pipeline reset mid-fetch")
                return
            }
            if Task.isCancelled {
                ...
                NSLog("AiInsight: fetch for \(key.label)/\(key.provider.rawValue) cancelled before result was applied")
                return
            }
            ...
            if cacheDate != cacheDateAtStart || cacheDate != currentCacheDate() {
                invalidateStaleDayCache()
                NSLog("AiInsight: dropping fetch result for \(key.label)/\(key.provider.rawValue) — calendar rolled mid-fetch")
                return
            }
            ...
            if Task.isCancelled { return }
            NSLog("AiInsight: fetch failed for \(key.label)/\(key.provider.rawValue): \(error)")
            if includeOptimize, cache[key] == nil {
                do {
                    ...
                    if Task.isCancelled { return }
                    NSLog("AiInsight: fallback fetch also failed: \(error)")
                }
            }
    ...
    func refreshQuietly(period: Period, day: String? = nil, force: Bool = false) async {
        ...
        if day == nil && period == .today, let age = todayPayloadAgeSeconds, age > 120 {
            NSLog("AiInsight: refreshing stale today status payload after %ds", age)
        }
        defer {
            inFlightKeys[key] = nil
        }
        do {
            ...
            if generationAtStart != payloadRefreshGeneration {
                NSLog("AiInsight: dropping quiet fetch result for \(key.label) — refresh pipeline reset mid-fetch")
                return
            }
            ...
            NSLog("AiInsight: quiet refresh failed for \(key.label): \(error)")
        }
    }
...
extension Notification.Name {
    static let codeBurnSubscriptionDisconnected = Notification.Name("com.aiinsight.subscriptionDisconnected")
}
