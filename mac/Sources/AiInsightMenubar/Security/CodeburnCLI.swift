...
/// Single entry point for spawning the `aiinsight` CLI. All callers route through here so the
...
enum AiInsightCLI {
    ...
    /// newlines) so a malicious `AIINSIGHT_BIN="aiinsight; rm -rf ~"` can't slip through.
    ...
    private static let userNodePaths: [String] = {
        ...
        if let entries = try? FileManager.default.contentsOfDirectory(atPath: versionsDir) {
            for entry in entries.sorted().reversed() {
                ...
                if FileManager.default.isExecutableFile(atPath: "\(bin)/aiinsight") {
                    paths.append(bin)
                    break
                }
            }
    ...
    private static let persistedPathFilename = "aiinsight-cli-path.v1"

    /// Returns the argv that launches the CLI. Dev override via `AIINSIGHT_BIN` is honoured only
    /// if every whitespace-delimited token passes `safeArgPattern`. Otherwise falls back to the
    /// plain `aiinsight` name (resolved via PATH).
    static func baseArgv() -> [String] {
        if ProcessInfo.processInfo.environment["AIINSIGHT_ALLOW_DEV_BIN"] == "1",
           let raw = ProcessInfo.processInfo.environment["AIINSIGHT_BIN"],
           !raw.isEmpty
        {
            let parts = raw.split(separator: " ", omittingEmptySubsequences: true).map(String.init)
            guard parts.allSatisfy(isSafe) else {
                NSLog("AiInsight: refusing unsafe AIINSIGHT_BIN; using installed aiinsight")
    ...
    private static func installedArgv() -> [String] {
        ...
        for candidate in (additionalPathEntries + userNodePaths).map({ "\($0)/aiinsight" }) {
    ...
    private static func persistedCLIPath() -> String? {
        ...
        let url = support
            .appendingPathComponent("AiInsight", isDirectory: true)
    ...
    static func makeProcess(subcommand: [String]) -> Process {
        ...
        // aiinsight subprocess parses 5-10x slower than the same command run from a
