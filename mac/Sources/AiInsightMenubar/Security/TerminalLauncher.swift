...
/// must be whitespace-joined argv where each token passes `AiInsightCLI.isSafe` (regex allowlist
...
enum TerminalLauncher {
    ...
    static func open(subcommand: [String]) {
        let argv = AiInsightCLI.baseArgv() + subcommand
        guard argv.allSatisfy(AiInsightCLI.isSafe) else {
            NSLog("AiInsight: refusing to open terminal with unsafe argv")
            return
        }
        ...
        let headless = AiInsightCLI.makeProcess(subcommand: subcommand)
        try? headless.run()
    }

    /// Launches `claude login` in Terminal.app so the user can complete the OAuth flow
    /// without leaving AiInsight. The command is a hardcoded literal -- no user input is
    ...
    static func openClaudeLogin() -> Bool {
        guard terminalPaths.contains(where: FileManager.default.fileExists(atPath:)) else {
            NSLog("AiInsight: Terminal.app not present; user must run `claude login` manually")
            return false
        }
    ...
    private static func runInTerminal(command: String, preValidated: Bool) {
        if !preValidated {
            ...
            guard tokens.allSatisfy(AiInsightCLI.isSafe) else {
                NSLog("AiInsight: refusing to run unvalidated command in Terminal")
                return
            }
        }
        let script = """
