...
///
/// Every cache file we touch (`~/Library/Caches/aiinsight-mac/fx-rates.json`,
/// `~/.cache/aiinsight/subscription-snapshots.json`, `~/.config/aiinsight/config.json`) is a
...
enum SafeFile {
    ...
    static func write(_ data: Data, to path: String, mode: mode_t = 0o600) throws {
        ...
        let tmpPath = parent + "/.aiinsight-" + UUID().uuidString + ".tmp"
    ...
    /// aiinsight processes (the CLI running in a terminal, say) block on the same file instead
