...
actor FXRateCache {
    ...
    private var cacheFilePath: String {
        ...
        return base
            .appendingPathComponent("aiinsight-mac", isDirectory: true)
    ...
    func rate(for code: String) async -> Double? {
        ...
        do {
            ...
            guard let fresh = decoded.rates[code],
                ...
                NSLog("AiInsight: discarding out-of-band FX rate for \(code)")
                return entries[code]?.rate
            }
...
/// Reads and writes the CLI's persisted currency config (~/.config/aiinsight/config.json).
/// Uses an on-disk flock so a concurrent `aiinsight currency ...` invocation from a terminal
...
enum CLICurrencyConfig {
    private static var configDir: String {
        (NSHomeDirectory() as NSString).appendingPathComponent(".config/aiinsight")
    }
    ...
    static func persist(code: String) {
        do {
            ...
            NSLog("AiInsight: failed to persist currency config: \(error)")
        }
    }
}

struct AiInsightCLIConfigStore {
    ...
    private var configDir: String {
        (homeDirectory as NSString).appendingPathComponent(".config/aiinsight")
    }
    ...
    func persistDevinAcuUsdRate(_ rate: Double) {
        ...
        do {
            ...
            NSLog("AiInsight: failed to persist Devin ACU config: \(error)")
        }
    }
}

enum CLIDevinConfig {
    private static let store = AiInsightCLIConfigStore()
