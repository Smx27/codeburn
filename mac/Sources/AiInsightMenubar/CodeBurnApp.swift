...
final class AppDelegate: NSObject, NSApplicationDelegate, NSPopoverDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        backgroundActivity = ProcessInfo.processInfo.beginActivity(
            ...
            reason: "AiInsight menubar background refresh"
        )
    ...
    private func recoverRefreshPipelineAfterInterruption(resetLoading: Bool, clearCache: Bool = false, reason: String) {
        ...
        if refreshTimer == nil || loopAge > refreshLoopWatchdogSeconds {
            if refreshTimer != nil {
                NSLog("AiInsight: refresh loop stale for %ds after %@ - restarting", Int(loopAge), reason)
            }
    ...
    private func removeLegacyRefreshAgent() {
        ...
        let destPath = "\(home)/Library/LaunchAgents/com.aiinsight.refresh.plist"
    ...
    private func registerLoginItemIfNeeded() {
        let key = "aiinsight.loginItemRegistered"
        ...
        do {
            ...
            NSLog("AiInsight: Login item registration failed: \(error)")
        }
    }
    ...
    private func clearStaleForceRefreshIfNeeded(now: Date = Date()) -> Bool {
        if forceRefreshTask != nil {
            guard let started = forceRefreshStartedAt else {
                NSLog("AiInsight: force refresh task had no start timestamp - clearing")
                forceRefreshTask?.cancel()
                forceRefreshTask = nil
                forceRefreshGeneration &+= 1
                store.resetLoadingState()
            ...
            NSLog("AiInsight: force refresh stuck for %ds - cancelling and restarting", Int(elapsed))
            forceRefreshTask?.cancel()
            forceRefreshTask = nil
            forceRefreshStartedAt = nil
            forceRefreshGeneration &+= 1
    ...
    private func clearStaleStatusPayloadRefreshIfNeeded(now: Date = Date()) -> Bool {
        if statusPayloadRefreshTask != nil {
            guard let started = statusPayloadRefreshStartedAt else {
                NSLog("AiInsight: today status refresh task had no start timestamp - clearing")
                statusPayloadRefreshTask?.cancel()
                statusPayloadRefreshTask = nil
                statusPayloadRefreshGeneration &+= 1
                return true
            ...
            NSLog("AiInsight: today status refresh stuck for %ds - cancelling", Int(elapsed))
            statusPayloadRefreshTask?.cancel()
            statusPayloadRefreshTask = nil
            statusPayloadRefreshStartedAt = nil
            statusPayloadRefreshGeneration &+= 1
    ...
    private func refreshStatusPayloadIfNeeded(reason: String, force: Bool = false) {
        ...
        if let age = store.menubarPayloadAgeSeconds, age > 120 {
            NSLog("AiInsight: status payload stale for %ds on %@ refresh", age, reason)
        }
    ...
    /// Loads the currency code persisted by `aiinsight currency` so a relaunch picks up where
    ...
    private func refreshPayloadForPopoverOpen() {
        ...
        if store.shouldResetInteractiveRefreshPipeline,
           let age = store.staleInteractivePayloadAgeSeconds {
            NSLog("AiInsight: popover opened with %ds stale payload cache - hard recovery", age)
        }
        Task { [weak self] in
            guard let self else { return }
    ...
    private var isCompact: Bool {
        UserDefaults.standard.bool(forKey: "AiInsightMenubarCompact")
    }
    ...
    private func setupStatusItem() {
        ...
        let flame = NSImage(systemSymbolName: "flame.fill", accessibilityDescription: "AiInsight")?
    ...
    private func refreshStatusButton() {
        ...
        button.toolTip = "AiInsight \(menubarPeriod.menubarMetricLabel)"
    ...
    private func persistBadgeStatusFile() {
        ...
        do {
            ...
            NSLog("AiInsight: failed to write badge status file: \(error)")
        }
    }
    ...
    private func showContextMenu(from button: NSStatusBarButton) {
        ...
        menu.addItem(.separator())
        let quitItem = NSMenuItem(title: "Quit AiInsight", action: #selector(quitApp), keyEquivalent: "q")
    ...
    @objc private func openSettings() {
        ...
        window.title = "AiInsight Settings"
    ...
    private func aiinsightAlertIcon() -> NSImage? {
        let config = NSImage.SymbolConfiguration(pointSize: 32, weight: .medium)
        guard let symbol = NSImage(systemSymbolName: "flame.fill", accessibilityDescription: "AiInsight")?
