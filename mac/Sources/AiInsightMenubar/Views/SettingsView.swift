        ...
        case .loaded, .transientFailure, .loading:
            Button("Disconnect") { showDisconnectConfirm = true }
                .confirmationDialog(
                    ...
                    Text("AiInsight will stop tracking quota and delete its local copy of your Claude credentials. Your Claude Code keychain entry is untouched — Claude Code keeps working.")
                }
        ...
        case .loaded, .transientFailure, .loading:
            Button("Disconnect") { showDisconnectConfirm = true }
                .confirmationDialog(
                    ...
                    Text("AiInsight will stop tracking quota and delete its local copy of your Codex credentials. Your ~/.codex/auth.json is untouched — Codex CLI keeps working.")
                }
        case .terminalFailure, .noCredentials, .failed:
...
private struct DevinSettingsTab: View {
    ...
    var body: some View {
        Form {
            ...
            Section {
                Text("AiInsight reads Devin ACU usage from local transcripts only after this rate is configured, then multiplies each step by the rate before reporting cost.")
                    .font(.system(size: 11))
                    .foregroundStyle(.secondary)
            } header: {
                Text("How it works")
    ...
    private func saveRate() {
        ...
        rateText = Self.format(rate)
        statusText = "Saved. Refresh AiInsight to recalculate Devin cost."
    }
    ...
    var body: some View {
        VStack(spacing: 14) {
            ...
            Text("Version \(appVersion) (\(buildVersion))")
                ...
                .foregroundStyle(.secondary)
            HStack(spacing: 10) {
                Link("GitHub", destination: URL(string: "https://github.com/priya/aiinsight")!)
                Link("Issues", destination: URL(string: "https://github.com/priya/aiinsight/issues")!)
            }
            .font(.system(size: 12))
        }
