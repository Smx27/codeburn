...
private let starBannerGitHubURL = URL(string: "https://github.com/priya/aiinsight")!
...
struct StarBanner: View {
    @AppStorage("aiinsight.starBannerDismissed") private var dismissed: Bool = false
...
struct FooterBar: View {
    ...
    /// Runs `aiinsight export` directly into ~/Downloads and reveals the result in Finder. CSV
    ...
    private func runExport(format: ExportFormat) {
        Task {
            ...
            let base = "aiinsight-\(formatter.string(from: Date()))"
            let outputPath = (downloads as NSString).appendingPathComponent(base + format.suffix)

            let process = AiInsightCLI.makeProcess(subcommand: [
                ...
                process.terminationHandler = { proc in
                    Task { @MainActor in
                        if proc.terminationStatus == 0 {
                            ...
                            NSLog("AiInsight: \(fmt.cliName.uppercased()) export exited with status \(proc.terminationStatus)")
                        }
                    }
                }
                try process.run()
            } catch {
                NSLog("AiInsight: \(format.cliName.uppercased()) export failed: \(error)")
            }
        }
    }
    ...
    /// Instant-feeling currency switch. Updates the symbol and any cached FX rate on the main
     ...
     /// background. CLI config is persisted so other aiinsight commands stay in sync.
