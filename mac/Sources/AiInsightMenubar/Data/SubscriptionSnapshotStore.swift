...
private func snapshotsCacheDir() -> String {
    return ProcessInfo.processInfo.environment["AIINSIGHT_CACHE_DIR"]
        ?? (NSHomeDirectory() as NSString).appendingPathComponent(".cache/aiinsight")
}
    ...
    static func record(_ snapshot: SubscriptionSnapshot) async {
        await SnapshotLock.shared.run {
            do {
                ...
                NSLog("AiInsight: snapshot record failed: \(error)")
            }
        }
    }
