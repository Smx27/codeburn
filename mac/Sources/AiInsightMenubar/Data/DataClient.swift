...
/// Runs the CLI via argv (no shell interpretation). See `AiInsightCLI` for why we never route
...
struct DataClient {
    ...
    private static func runCLI(subcommand: [String]) async throws -> ProcessResult {
        ...
        let process = AiInsightCLI.makeProcess(subcommand: subcommand)
    ...
    static func runProcess(_ process: Process,
        ...
        timeoutTimer.setEventHandler {
            if process.isRunning {
                NSLog("AiInsight: CLI subprocess timed out after %llus for %@ — terminating",
    ...
    private static func drain(_ handle: FileHandle, limit: Int) async -> Data {
        ...
        if flags >= 0 {
            ...
            NSLog("AiInsight: fcntl F_GETFL failed on fd %d, drain may block", fd)
        }
        ...
        while buffer.count < limit && !Task.isCancelled {
            ...
            if n > 0 {
                ...
                NSLog("AiInsight: drain read() failed on fd %d: errno %d", fd, errno)
                break
            }
        }
        return buffer
