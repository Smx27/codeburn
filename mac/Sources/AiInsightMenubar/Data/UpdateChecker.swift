...
private let releasesAPI = "https://api.github.com/repos/priya/aiinsight/releases?per_page=20"
...
final class UpdateChecker {
    ...
    var cliUpdateCommand: String {
        let argv = AiInsightCLI.baseArgv()
        let path = argv.first ?? ""
        if path.contains("/homebrew/") { return "brew upgrade aiinsight" }
        return "npm update -g aiinsight"
    }
    ...
    func check() async {
        ...
        request.setValue("aiinsight-menubar-updater", forHTTPHeaderField: "User-Agent")
        ...
        do {
            ...
            let version = resolved.asset.name
                .replacingOccurrences(of: "AiInsightMenubar-", with: "")
            ...
            NSLog("AiInsight: update check failed: \(error)")
        }
    }
    ...
    nonisolated static func queryInstalledCliVersion() -> String? {
        let process = AiInsightCLI.makeProcess(subcommand: ["--version"])
        let pipe = Pipe()
    ...
    nonisolated static func resolveLatestMenubarRelease(in releases: [GitHubRelease]) -> (release: GitHubRelease, asset: GitHubAsset)? {
        for release in releases where release.tag_name.hasPrefix("mac-v") {
            guard let asset = release.assets.first(where: {
                $0.name.hasPrefix("AiInsightMenubar-v") && $0.name.hasSuffix(".zip")
            }) else { continue }
    ...
    func performUpdate() {
        ...
        let process = AiInsightCLI.makeProcess(subcommand: ["menubar", "--force"])
        let errPipe = Pipe()
        ...
        let timeoutTask = Task.detached(priority: .utility) {
            ...
            if process.isRunning {
                NSLog("AiInsight: update subprocess timed out after %llus - terminating", updateTimeoutSeconds)
                process.terminate()
            }
        }
        ...
        process.terminationHandler = { [weak self] proc in
            ...
            Task { @MainActor in
                ...
                if proc.terminationStatus != 0 {
                    ...
                    NSLog("AiInsight: update failed (exit \(proc.terminationStatus)): \(stderr)")
                } else {
        ...
        do {
            ...
            NSLog("AiInsight: update spawn failed: \(error)")
        }
    }
