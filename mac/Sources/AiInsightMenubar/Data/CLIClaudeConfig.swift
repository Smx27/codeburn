...
/// `~/.config/aiinsight/config.json`. The menubar is a GUI app that doesn't
...
/// accounts). Persisting to the shared config file instead means every `aiinsight`
/// invocation honors the list — whether the menubar spawns it directly or the
/// user launches `aiinsight report` in a terminal — without injecting environment
...
/// `aiinsight` write from a terminal can't race the menubar and drop the other's
...
enum CLIClaudeConfig {
    ...
    static func persist(dirs: [String]) {
        ...
        do {
            ...
            NSLog("AiInsight: failed to persist claudeConfigDirs config: \(error)")
        }
    }
}
