...
/// Connect we cache a copy under ~/Library/Application Support/AiInsight so
/// we keep using rotated tokens after refresh.
enum CodexCredentialStore {
    private static let bootstrapCompletedKey = "aiinsight.codex.bootstrapCompleted"
    ...
    private static func refreshAndPersist(record: CredentialRecord) async throws -> CredentialRecord {
        ...
        do {
            ...
            NSLog("AiInsight: codex cache write failed during refresh rotation: %@", String(describing: error))
        }
        return updated
    }
}
