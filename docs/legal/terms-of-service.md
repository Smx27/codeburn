# Terms of Service

Terms of service for AIInsight.

**Last Updated:** June 20, 2026

## Agreement to Terms

By accessing or using AIInsight, you agree to be bound by these Terms of Service. If you do not agree, do not use the service.

## Description of Service

AIInsight is an AI coding cost management platform that:
- Tracks token usage across AI coding tools
- Provides cost analytics and insights
- Syncs usage data to a self-hosted dashboard
- Supports multiple AI coding providers

## Account Registration

### Requirements

- You must be at least 13 years old
- You must provide accurate information
- You must secure your account credentials

### Responsibilities

- Maintain account security
- Notify us of unauthorized access
- One account per person

## Acceptable Use

### Permitted Uses

- Track your AI coding usage
- Analyze your costs
- Share with your team
- Integrate with your workflows

### Prohibited Uses

- Violate any laws or regulations
- Infringe on intellectual property rights
- Transmit harmful or malicious code
- Attempt to access unauthorized data
- Resell or redistribute the service
- Use for competitive analysis

## Data Ownership

### Your Data

- You retain ownership of all data
- We do not access your prompts or code
- You can export or delete your data anytime

### Our Data

- We own the service and its components
- We own aggregated, anonymized data
- We own our intellectual property

## Service Availability

### Uptime

- We target 99.9% uptime
- Planned maintenance announced in advance
- No guarantee for third-party services

### Limitations

- Service may be temporarily unavailable
- We may modify features with notice
- We may suspend service for violations

## Pricing and Payment

### Free Tier

- Limited usage included
- No credit card required
- Subject to rate limits

### Paid Plans

- Billed monthly or annually
- Auto-renewal unless cancelled
- No refunds for partial months

### Changes

- We may change pricing with 30 days notice
- Existing plans honored until renewal
- New pricing applies at renewal

## Intellectual Property

### Our IP

- AIInsight software and services
- Documentation and materials
- Trademarks and logos

### Your IP

- Your code and data
- Your trademarks and logos
- Your integrations

## Limitation of Liability

### No Warranty

- Service provided "as is"
- No guarantee of fitness for purpose
- No guarantee of uninterrupted service

### Limitation

- Not liable for indirect damages
- Not liable for data loss
- Liability limited to fees paid

## Indemnification

You agree to indemnify us from claims arising from:
- Your use of the service
- Your violation of these terms
- Your violation of third-party rights

## Termination

### By You

- Cancel anytime through dashboard
- Data retained for 30 days
- Export data before cancellation

### By Us

- For violation of these terms
- For non-payment
- With 30 days notice

### Effect

- Access terminated immediately
- Data deleted after retention period
- Outstanding fees remain due

## Dispute Resolution

### Governing Law

- Laws of the State of California
- United States

### Disputes

- First, attempt informal resolution
- Then, binding arbitration
- Class action waiver

## Changes to Terms

- We may update these terms
- Material changes notified via email
- Continued use constitutes acceptance

## Contact Us

If you have questions about these Terms, please contact us at:
- Email: legal@aiinsight.dev
- GitHub: https://github.com/getagentseal/codeburn/issues

## Related Documentation

- [Privacy Policy](privacy-policy.md) — Data practices
- [Acceptable Use Policy](acceptable-use-policy.md) — Usage guidelines
