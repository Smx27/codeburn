...
`kimi.json` maps each work-directory hash back to the original working path. AiInsight uses that to display the project basename; if the metadata file is missing, the hash directory name is used.

## Storage Format

AiInsight reads `wire.jsonl`. Each data line is a persisted wire record:
...
- Kimi's official `TokenUsage` separates `input_other`, `input_cache_read`, `input_cache_creation`, and `output`. AiInsight maps those directly into input, cache read, cache write, and output.
- The current Kimi wire schema does not persist the model on every usage update. AiInsight uses `KIMI_MODEL_NAME` when set, then the active `~/.kimi/config.toml` default model, then `kimi-auto`.
...
3. If tools are missing, check whether Kimi emitted `ToolCall`, `ToolCallRequest`, or nested `SubagentEvent`; AiInsight intentionally counts subagent wire files separately to avoid double-counting parent mirrors.
