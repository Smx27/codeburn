...
Subagent traces are stored under a parent session's `agents/` folder with the same `meta.json` / `messages.jsonl` shape, so AiInsight scans those one level down as separate sessions.

## Caching

Current Vibe local logs do not expose cache-read/cache-write token fields, so
AiInsight reports cache token counts as `0`. When `meta.json.stats.session_cost`
is present, AiInsight uses that session total instead of re-estimating from
...
- **Tool calls come from messages.** Assistant `tool_calls[*].function.name` is normalized to the standard AiInsight names (`bash` to `Bash`, `search_replace` to `Edit`, etc.). Bash commands are extracted from `function.arguments.command`.
